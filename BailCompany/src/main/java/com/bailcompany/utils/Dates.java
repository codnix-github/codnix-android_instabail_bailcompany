...
public class Dates {
    ...
    //diffUnit= m=minute, s=seconds, h=hours
    public static int getDifferentFromCurrentTime(DateTime sourceDate, char diffUnit) {
        DateTime dateLocalZone = sourceDate.withZone(DateTimeZone.getDefault());
        DateTime dateUTC = sourceDate.withZone(DateTimeZone.UTC);
        DateTime nowUTC = DateTime.now(DateTimeZone.UTC);
        int differentInSec = Seconds.secondsBetween(dateUTC, nowUTC).getSeconds();
        if (diffUnit == 's') {
            return differentInSec;
        }
        if (diffUnit == 'm') {
            return differentInSec / 60;
        }
        if (diffUnit == 'h') {
            return differentInSec / (60 * 60);
        }
