...
import com.bailcompany.service.DownloadFileAdapter;
...
public class DropboxDownload extends Activity implements OnItemClickListener {
	...
	private void checkAppKeySetup() {
		if (WebAccess.DROPBOX_APP_KEY.startsWith("CHANGE")
				|| WebAccess.DROPBOX_APP_SECRET.startsWith("CHANGE")) {
			showToast("You must apply for an app key and secret from developers.dropbox.com, and add them to the DBRoulette ap before trying it.");
			finish();
			return;
		}
		Intent testIntent = new Intent(Intent.ACTION_VIEW);
		String scheme = "db-" + WebAccess.DROPBOX_APP_KEY;
	...
	private boolean downloadDropboxFile(Entry fileSelected) {// , String
		...
		if (!dir.exists())
			dir.mkdirs();
		try {
			File localFile = new File(dir + "/" + Const.getUniqueIdforImage()
...
//			}
	...
	private void showFileExitsDialog(final Entry fileSelected,
		...
		alertBuilder.setMessage(WebAccess.OVERRIDEMSG);
